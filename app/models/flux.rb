...
class Flux < ActiveRecord::Base #CachedModel 
  ...
  def headspace
    if 'Z' == incubation.lid 
      ...
      return 1/4 * Math::PI * (26 + 0.094697 * )^2 * (incubation.avg_height_cm - 1) # one cm from the top of the bucket to the mark
    else
      begin
